# Manifest

<!-- Generated by scripts/update-manifest.sh — do not edit by hand. -->

- `.github/CODEOWNERS`
- `.github/pull_request_template.md`
- `.github/workflows/ci.yml`
- `.gitignore`
- `CHANGELOG.md`
- `CLAUDE.md`
- `DOCUMENTATION.html`
- `LICENSE`
- `README.html`
- `README.md`
- `VERSION`
- `agents/documenter.md`
- `agents/implementer.md`
- `agents/leader.md`
- `agents/optional/browser-tester.md`
- `agents/reviewer.md`
- `agents/spec-author.md`
- `decisions/answers.template.md`
- `decisions/architecture-decisions.template.md`
- `decisions/rejected-options.template.md`
- `decisions/workflow-decisions.template.md`
- `hooks/examples/block-implementation-before-approval.sh`
- `hooks/examples/failure-learning/README.md`
- `hooks/examples/failure-learning/suggest-failure-learning.sh`
- `hooks/examples/pre-compact-capture/README.md`
- `hooks/examples/pre-compact-capture/post-compact-reorient.sh`
- `hooks/examples/pre-compact-capture/pre-compact-capture.sh`
- `hooks/examples/run-tests-after-edit.sh`
- `hooks/examples/spec-drift/README.md`
- `hooks/examples/spec-drift/spec-drift.sh`
- `hooks/examples/targeted-validation/README.md`
- `hooks/examples/targeted-validation/targeted-validation.sh`
- `hooks/examples/validate-spec-before-status-change.sh`
- `hooks/hooks-policy.md`
- `hooks/recommended-hooks.md`
- `hooks/settings-snippets.md`
- `instructions.md`
- `manifest.md`
- `mcps/github.md`
- `mcps/jira.md`
- `mcps/linear.md`
- `mcps/local-filesystem.md`
- `mcps/mcp-policy.md`
- `mcps/memory.md`
- `mcps/playwright-policy.md`
- `output-project-structure.md`
- `questions.md`
- `reference/autonomy-policy.md`
- `reference/claude-code-primitives.md`
- `reference/cli-vs-mcp-policy.md`
- `reference/context-economy.md`
- `reference/deep-review-policy.md`
- `reference/dependency-freshness-policy.md`
- `reference/harness-engineering.md`
- `reference/memory-policy.md`
- `reference/sdd-theory.md`
- `reference/session-recovery.md`
- `scripts/init.sh`
- `scripts/render-spec.mjs`
- `scripts/render_spec.py`
- `scripts/run-lint.sh.template`
- `scripts/run-tests.sh.template`
- `scripts/update-manifest.sh`
- `scripts/validate-sdd-structure.ps1`
- `scripts/validate-sdd-structure.sh`
- `skills/optional/README.md`
- `skills/optional/context-audit/SKILL.md`
- `skills/optional/decision-log/SKILL.md`
- `skills/optional/dependency-freshness/SKILL.md`
- `skills/optional/documentation-update/SKILL.md`
- `skills/optional/failure-learning/SKILL.md`
- `skills/optional/git-discipline/SKILL.md`
- `skills/optional/project-map/SKILL.md`
- `skills/optional/run-and-verify/SKILL.md`
- `skills/optional/spec-from-screenshot/SKILL.md`
- `skills/optional/ui-qa/SKILL.md`
- `skills/sdd-update/SKILL.md`
- `skills/sdd-workflow/SKILL.md`
- `skills/sdd-workflow/assumptions-policy.md`
- `skills/sdd-workflow/examples.md`
- `skills/sdd-workflow/intake-from-functional-doc.md`
- `skills/sdd-workflow/open-questions-policy.md`
- `skills/sdd-workflow/review-checklist.md`
- `skills/sdd-workflow/spec-format.md`
- `skills/sdd-workflow/task-state-machine.md`
- `skills/sdd-workflow/workflow.md`
- `specs/example-feature/acceptance-tests.html`
- `specs/example-feature/acceptance-tests.md`
- `specs/example-feature/assumptions.html`
- `specs/example-feature/assumptions.md`
- `specs/example-feature/design.html`
- `specs/example-feature/design.md`
- `specs/example-feature/open-questions.html`
- `specs/example-feature/open-questions.md`
- `specs/example-feature/requirements.html`
- `specs/example-feature/requirements.md`
- `specs/example-feature/review.html`
- `specs/example-feature/review.md`
- `specs/example-feature/tasks.html`
- `specs/example-feature/tasks.md`
- `templates/.claude/hooks/README.md.template`
- `templates/.claude/settings.json.template`
- `templates/.mcp.json.template`
- `templates/CLAUDE.md.template`
- `templates/architecture.md.template`
- `templates/conventions.md.template`
- `templates/functional/functional-brief.md.template`
- `templates/git/commit-message.template`
- `templates/git/pr-description.md.template`
- `templates/git/release-note.md.template`
- `templates/history.md.template`
- `templates/memory/failure-learning-entry.md`
- `templates/project-map.md.template`
- `templates/run-and-verify.md.template`
- `templates/sdd-kit-manifest.schema.md`
- `templates/specs/README.md`
- `templates/specs/acceptance-tests.md.template`
- `templates/specs/assumptions.md.template`
- `templates/specs/design.md.template`
- `templates/specs/open-questions.md.template`
- `templates/specs/requirements.md.template`
- `templates/specs/review.md.template`
- `templates/specs/spec-shell.html.template`
- `templates/specs/spec.css`
- `templates/specs/spec.js`
- `templates/specs/tasks.md.template`
- `templates/tasks.json.template`
- `usage-prompts.md`
